...
@ti.data_oriented
class fem_mgpcg:
    def __init__(self, nelx, nely, fixed_dof, K, F, dim=2, n_mg_levels = 4, dtype=ti.f32):
        ...
        self.dim = dim
        self.real = dtype
        self.fixed_dof = fixed_dof  # fixed dofs: ti.Vector
    ...
    def multigrid_init(self):
        for i in range(self.ndof):
            self.r[0][i] = self.b[0][i] # r = b - A * x = b
            self.z[0][i] = 0.
            self.Ap[i] = 0.
            self.p[i] = 0.
            self.x[i] = 0.
    ...
    def solve(self, U, max_iters=-1, eps=1e-12, abs_tol=1e-12, rel_tol=1e-12, verbose=True):
        ...
        self.multigrid_init()
        initial_rTr = self.reduce(self.r[0], self.r[0])  # Used to check convergence
        ...
        old_zTr = self.reduce(self.r[0], self.z[0])

        # cg iteration
        iter = 0
        while max_iters == -1 or iter < max_iters:
            self.cg_compute_Ap(self.A[0], self.p)
            ...
            rTr = self.reduce(self.r[0], self.r[0])  # check convergence
            if verbose:
            ...
            new_zTr = self.reduce(self.r[0], self.z[0])
